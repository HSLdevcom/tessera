...
module.exports = function(tilelive, options) {
  ...
  var populateHeaders = function(headers, params, extras) {
    ...
    Object.keys(templates).forEach(function(name) {
      ...
      // Prefer manually given headers over template
      if (val && !headers[name.toLowerCase()]) {
  ...
  app.get("/index.json", function(req, res, next) {
    ...
    return tilelive.load(uri, function(err, source) {
      ...
      return tessera.getInfo(source, function(err, info) {
        ...
        var protocol = (headers['x-forwarded-proto'] || '').match(/^[a-z]+$/) ? headers['x-forwarded-proto'] : req.protocol;

        var host = headers['x-host'] || headers['x-forwarded-host'] || headers.host;

        var searchParams = url.parse(req.url).search;
        ...
        res.set(populateHeaders({'cache-control': 'no-store'}, params, { 200: true }));
